...
//!FUNCTIONS
#define DRAWBUFFER SDL_UpdateTexture(window->framebuffer, nullptr, pixelbuffer.data, pixelbuffer.width<<2);\
	SDL_RenderCopy(window->renderer, window->framebuffer, nullptr, nullptr);
#define INITWINDOW SDL_Init(SDL_INIT_VIDEO); \
	window=SDL_CreateWindow(title, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, width, height, SDL_WINDOW_SHOWN); renderer=SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED);
